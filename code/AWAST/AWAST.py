...
def build_parser():
    ...
    parser.add_argument('--folder_styles',
            dest='folder_styles', help='folder of style image',
...
def main():
    ...
    print("Network load finish.")

    content_image = imread(options.content)

    style_images = []
    style_name = []
    for root, dirs, files in os.walk(options.folder_styles):
        for file in files:
            if file.find(".jpg") == -1 and file.find(".png") == -1:
                continue
            style_images.append(imread(os.path.join(root, file)))
            style_name.append(file)
